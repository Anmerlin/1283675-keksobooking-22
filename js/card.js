...
createSimilarCards.forEach(({author, offer}) => {
  ...
  features.forEach((currentValue) => {
    if (!checkClassValueInArray(currentValue, dataOffer.features)) {
      currentValue.remove();
    }
    return currentValue;
  });
  ...
  createImagesCardFromArray(photo, dataOffer.photos); // В блок .popup__photos выведите все фотографии из списка offer.photos. Каждая из строк массива photos должна записываться как src соответствующего изображения.
